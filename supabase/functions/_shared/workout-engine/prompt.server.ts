...
- VARIETY RULE (automatic rejection if broken): never build a session out of one movement wearing different clothes. The SAME movement family (bench press / incline bench press / decline bench press / guillotine press are ONE family; squat variations are ONE family) may appear at most TWICE in 💪 Main Workout and ⚡ Finisher combined, and never twice in a row. Never write the same exercise id twice anywhere in the session.
- IMPLEMENT VARIETY: when the athlete has a gym, the session must travel across the equipment they actually have — barbell, dumbbells, kettlebells, cables and selectorized machines. A gym chest session looks like: bench press, then incline dumbbell press, then a machine or cable movement (pec deck, cable fly, chest press machine), then dips or push-ups. A gym session that uses only one implement, or only one station, is wrong even if every line is legal. Use at least THREE different implements whenever three are available, and at least one machine or cable station in STRENGTH / MUSCLE BUILDING gym sessions.
- LOGICAL FLOW: heaviest and most technical compound first, then a second compound at a different angle, then isolation or machine work, then the bodyweight or finishing work. The Finisher uses DIFFERENT movements from the Main Workout, not the same lift again.
