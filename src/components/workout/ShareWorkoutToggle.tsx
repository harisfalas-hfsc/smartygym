...
export function ShareWorkoutToggle({ workout, compact, onChanged }: ShareWorkoutToggleProps) {
  ...
  /**
   * Asks the backend to create the cover picture. The backend saves it itself,
   * so the picture still appears if the member leaves the page meanwhile.
   */
  const generateCover = async () => {
    if (workout.image_url) return;
    try {
      await supabase.functions.invoke("generate-shared-workout-cover", {
